...
class WassersteinGPLoss:
    """Enhanced Wasserstein Loss with Gradient Penalty"""
    ...
    def critic_loss(self, critic, real_samples, fake_samples, real_labels, fake_labels, device):
        """Calculate enhanced critic loss"""
        # Real samples
        ...
        # Gradient penalty
        gp = self.gradient_penalty(critic, real_samples, fake_samples, real_labels, device)
        ...
        return critic_loss, real_loss, fake_loss, gp, wasserstein_distance
...
def train_enhanced_gan_with_resume_modified(dataset_key, config, resume_from_checkpoint=True, num_epochs=100):
    ...
    """Enhanced training with 5-epoch checkpoints and graceful interrupt handling - FIXED for variable scoping"""
    ...
    if dataset_key == 'mnist':
        ...
        # num_epochs = 100  # Restore full training
    else:  # cifar10
        learning_rate_g = 0.0001
        learning_rate_d = 0.0004
        # num_epochs = 100
    ...
    # ============================================================================
    # FIX: Load dataset BEFORE using train_loader - this was the missing piece!
    # ============================================================================
    ...
    try:
        # Use num_workers=2 for normal operation, but fall back to 0 if multiprocessing issues
        train_loader = torch.utils.data.DataLoader(
            ...
            pin_memory=False if device_type == "mps" else True  # Disable pin_memory for MPS
        )
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            batch_size=batch_size,
            shuffle=True,
            num_workers=0  # Single-threaded for maximum stability
        )

    print(f"✅ Dataset loaded successfully!")
    print(f"   📊 Training samples: {len(train_dataset):,}")
    print(f"   📦 Batch size: {batch_size}")
    # NOW we can safely use train_loader here
    ...
    # Fixed samples for evaluation - ensure they're on the correct device
    ...
    print(f"\n🚀 STARTING/RESUMING ENHANCED TRAINING WITH AUTO-SAVE...")
    ...
    try:
        for epoch in epoch_pbar:
            ...
            print(f"\n\n📅 EPOCH {epoch+1}/{num_epochs} - Enhanced Training")
            ...
            for i, (real_imgs, labels) in enumerate(batch_pbar):
                ...
                for critic_iter in range(n_critic):
                    ...
                    # Generate fake images - ensure tensors are on correct device
                    ...
                    d_loss, d_real, d_fake, gp, wd = wgan_loss.critic_loss(
                ...
                # Update EMA - this is where the device error was happening
                try:
                ...
                if i % 25 == 0 or i < 5 or i == len(train_loader) - 1:
                    ...
                    print_step_details(i+1, len(train_loader), stats, progress_tracker,
                                       epoch+1, num_epochs)
            ...
            print(f"\n🎉 EPOCH {epoch+1} COMPREHENSIVE SUMMARY:")
            print("=" * 80)
            print(f"⏱️  Epoch Time: {epoch_time:.1f}s (Avg Batch: {avg_batch_time:.3f}s)")
            ...
            print(f"   💜 EMA Quality Score: {ema_generator.quality_score:.4f}")
            ...
            # 🆕 SAVE CHECKPOINT EVERY 5 EPOCHS (instead of 25)
            if (epoch + 1) % 5 == 0:
